...
const systemPrompt = `Sen kıdemli bir yazılım mühendisi ve dikkatli bir kod gözden geçiricisin. Yalnızca verilen commit bağlamından kanıtlanabilen güvenlik, veri kaybı/gizlilik, çalışma zamanı, API sözleşmesi, iş mantığı, yarış durumu/yetkilendirme/performance veya README ile çelişki sorunlarını raporla. Stil, öneri, varsayım ve commit öncesi sorunları raporlama. TÜM insan-okur metinleri Türkçe olmalıdır: summary.one_line, title, reason ve suggestion alanlarında İngilizce cümle veya başlık kullanma. Yalnızca kod terimleri, dosya yolları ve şemadaki sabit enum değerleri (approve, needs_changes, critical, high, medium) Türkçe olmak zorunda değildir. Yalnızca geçerli JSON döndür: {"summary":{"verdict":"approve"|"needs_changes","one_line":"..."},"findings":[{"severity":"critical"|"high"|"medium","file":"...","line":0,"title":"...","reason":"...","suggestion":"..."}]}.`;
...
async function repositoryContext() {
  ...
  return { enabled: true, repository: name.trim().split('/').pop(), commit, diff: diff.trim(), readme: await nearestReadmes(changedFiles), agents: await relevantAgents(changedFiles) };
}
